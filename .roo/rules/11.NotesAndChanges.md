...
### **Phase 3 Part 3 - Technical Notes & Changes**

*   **API Expansion:**
    *   Introduced new `PUT` and `DELETE` endpoints for `/api/tags/` and `/api/collections/` to handle renaming and deletion.

*   **Server-Side Logic:**
    *   Implemented robust backend logic that iterates through the entire card library on the file system to propagate tag/collection renames and deletions, ensuring data consistency.
    *   Added `collections.json` as a new persistent data store for managing collections globally.

*   **Client-Side State Management (Zustand):**
    *   The global `cardStore` was extended to manage the state for the new management pages.
    *   Added new actions (`updateTag`, `deleteTag`, `fetchCollections`, etc.) to communicate with the new API endpoints.

*   **UI/UX & Routing:**
    *   Created two new pages: `TagManagement` and `CollectionManagement`, providing dedicated UIs for these features.
    *   Added primary navigation links for "Tags" and "Collections" to the main header for easy access.
    *   Implemented in-line editing and confirmation dialogs on the new pages for a streamlined management experience.